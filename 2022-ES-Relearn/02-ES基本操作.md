...
ES默认情况下只返回TOP10、order by _score desc的数据，ES采取的是逻辑分页，比如我要查第91-100的数据就要传90,10这样的分页参数，ES实际上是取了前100个数据，然后截取第91-第100个返回。

还记得第一次学ES的时候，就学到ES的数据是基于shards存储的，每1个索引都会被拆分成不同的部分存储在1个shard里（有点类似Kafka的partition）。那么问题来了，索引在不同shard里的文档数据其实是无序的，那么在不同shard里共同查询出第91-第100个数据该如何做呢？这其实有点类似分库分表的分页问题，在数据库层面可以通过sharding-jdbc和mycat处理，那ES呢？它要怎么做？

实际上，ES会在**每个shard都根据排序规则查前100条数据**出来，最终将每1个shard的结果整合到一个结果集，在这个结果集里重新排序，最后取前100个数据，再截取第91-第100条数据出来返回。

如shard数量=5，需要查91-100的数据，ES会从5个shard里根据排序规则分别查出前100个该索引的文档，整合到一个结果集（数量=500），再从结果集里的500个文档里重新排序，取出前100个数据，截取第91-第100返回。

如果分页条件过于苛刻（如pageSize=100，pageNo=10000），这对内存的消耗其实挺大的，所以ES做了个很暴力的限制：**单个shard的查询上限不能超过10000（当然，这可以通过修改配置来解决，但不推荐）**。所以对于大页数展示的需求，实现起来要考虑其他方案。
